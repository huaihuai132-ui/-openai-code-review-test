<template>
  <el-row :gutter="20">
    <el-col :span="16">
      <ContentWrap title="用车信息">
        <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="80px">
          <el-form-item label="用车时间" prop="useTime">
            <el-date-picker v-model="formData.useTime" clearable placeholder="请选择用车时间" type="datetime"
              value-format="x" />
          </el-form-item>
          <el-form-item label="车辆选择" prop="carNo">
            <el-select v-model="formData.carNo" clearable placeholder="请选择车辆">
              <el-option label="电车1" :value="1" />
              <el-option label="油车2" :value="2" />
            </el-select>
          </el-form-item>
          <el-form-item label="用车原因" prop="reason">
            <el-input v-model="formData.reason" placeholder="请输入用车原因" type="textarea" :rows="4" />
          </el-form-item>
...
defineOptions({ name: 'BpmOAUseCarCreate' })
...
const formRules = reactive({
  useTime: [{ required: true, message: '用车时间不能为空', trigger: 'change' }],
  carNo: [{ required: true, message: '车辆选择不能为空', trigger: 'change' }],
  reason: [{ required: true, message: '用车原因不能为空', trigger: 'change' }]
})
const formRef = ref() // 表单 Ref
const fileUploadRef = ref() // 文件上传组件 Ref

// 审批相关：变量
const processDefineKey = 'oa_form_use_car' // 流程定义 Key
...
const submitForm = async () => {
  ...
  try {
    const data = { ...formData.value } as unknown as UseCarApi.UseCarVO
    ...
    await UseCarApi.createUseCar(data)
...
const getApprovalDetail = async () => {
  try {
    const data = await ProcessInstanceApi.getApprovalDetail({
      ...
      processVariablesStr: JSON.stringify({}) // 用车申请相对简单，无需传递特殊变量
    })
...
const initProcessDefinition = async () => {
  ...
  if (!processDefinitionDetail) {
    message.error('OA 用车的流程模型未配置，请检查！')
    return
  }
